...
#     #  归一化
#     img = img / 255.0
#     for i in range(colorDict_GRAY.shape[0]):  #  有colorDict_GRAY参数，是各类别颜色字典
...
# //////////2.2 获取颜色字典
#  (我的毕设是二分类，所以可能不需要运行这一部分)
'''
 各类别的颜色字典，比如我们如果只有两类的话，colorDict_GRAY = (0, 255)
  用程序自动获取colorDict_GRAY
'''
...
def color_dict(labelFolder, classNum):
    ...
    for i in range(len(ImageNameList)):
        ...
        # unique:去重 升序。https://numpy.org/doc/stable/reference/generated/numpy.unique.html
        #  将第i个像素矩阵的 唯一值 添加到colorDict中
...
#  Test 测试数据生成器
#  test_image_path 测试数据路径
#  resize_shape resize大小
def testGenerator(test_image_path, resize_shape = None):
    imageList = os.listdir(test_image_path)
    for i in range(len(imageList)):
        img = readTif(test_image_path + "\\" + imageList[i])
        img = img.swapaxes(1, 0)
        img = img.swapaxes(1, 2)
        #  归一化
        img = img / 255.0
...
# labelFolder = "F:\Files auto-update\\0 Graduation Paper\\2Model\Coding\Data\Tags"
# classNum = 2
# color_dict(labelFolder, classNum)