...
# 显示混淆矩阵
def plot_confuse_data(y_true, y_pred):
    classes = range(0,2)
...
#################################################################
#  标签图像文件夹
LabelPath = "H:\CXY2022\\0UnetTrainData\Active\\val_label"
#  预测图像文件夹
PredictPath = "H:\CXY2022\\0UnetTrainData\Active\predict\FC1e-5"
#  类别数目(包括背景)
classNum = 2
#################################################################
...
# predict_all.head()
# predict_all.tail()
print(label_all, predict_all)

# ## 自行添加的混淆矩阵算法
# confusionMatrix =  confusion_matrix(label_all, predict_all, labels=[0,1,2])
# print(confusionMatrix)
plot_confuse_data(label_all, predict_all)
